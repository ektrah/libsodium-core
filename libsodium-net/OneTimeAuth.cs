...
namespace Sodium
{
  ...
  public static class OneTimeAuth
  {
    ...
    [DllImport("libsodium-4.dll", EntryPoint = "crypto_onetimeauth", CallingConvention = CallingConvention.Cdecl)]
    private static extern int _Sign(byte[] buffer, byte[] message, long messageLength, byte[] key);

    [DllImport("libsodium-4.dll", EntryPoint = "crypto_onetimeauth_verify", CallingConvention = CallingConvention.Cdecl)]
