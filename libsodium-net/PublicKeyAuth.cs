...
namespace Sodium
{
  ...
  public static class PublicKeyAuth
  {
    ...
    [DllImport("libsodium-4.dll", EntryPoint = "crypto_sign_keypair", CallingConvention = CallingConvention.Cdecl)]
    private static extern int _GenerateKeyPair(byte[] publicKey, byte[] secretKey);

    [DllImport("libsodium-4.dll", EntryPoint = "crypto_sign", CallingConvention = CallingConvention.Cdecl)]
    private static extern int _Sign(byte[] buffer, ref long bufferLength, byte[] message, long messageLength, byte[] key);

    [DllImport("libsodium-4.dll", EntryPoint = "crypto_sign_open", CallingConvention = CallingConvention.Cdecl)]
