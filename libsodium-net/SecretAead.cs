...
namespace Sodium
{
    ...
    public static class SecretAead
    {
        ...
        public static byte[] Encrypt(byte[] message, byte[] nonce, byte[] key, byte[] additionalData = null)
        {
            ...
            {
                throw new AdditionalDataOutOfRangeException(string.Format("additionalData must be between {0} and {1} bytes in length.", 0, ABYTES));
            }

            var cipher = new byte[message.Length + ABYTES];
            var bin = Marshal.AllocHGlobal(cipher.Length);
            long cipherLength;
            var encrypt = DynamicInvoke.GetDynamicInvoke<_Encrypt>("crypto_aead_chacha20poly1305_encrypt", SodiumCore.LibraryName());
            var ret = encrypt(bin, out cipherLength, message, message.Length, additionalData, additionalData.Length, null, nonce, key);
        ...
        public static byte[] Decrypt(byte[] cipher, byte[] nonce, byte[] key, byte[] additionalData = null)
        {
            ...
            long messageLength;
            var decrypt = DynamicInvoke.GetDynamicInvoke<_Decrypt>("crypto_aead_chacha20poly1305_decrypt", SodiumCore.LibraryName());
            var ret = decrypt(bin, out messageLength, null, cipher, cipher.Length, additionalData, additionalData.Length, nonce, key);
        ...
        private delegate int _Encrypt(IntPtr cipher, out long cipherLength, byte[] message, long messageLength, byte[] additionalData, long additionalDataLength, byte[] nsec, byte[] nonce, byte[] key);
        //crypto_aead_chacha20poly1305_decrypt
        private delegate int _Decrypt(IntPtr message, out long messageLength, byte[] nsec, byte[] cipher, long cipherLength, byte[] additionalData, long additionalDataLength, byte[] nonce, byte[] key);
    }
}