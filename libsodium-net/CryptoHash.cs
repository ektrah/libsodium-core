...
namespace Sodium
{
  ...
  public class CryptoHash
  {
    ...
    [DllImport("libsodium", EntryPoint = "crypto_hash", CallingConvention = CallingConvention.Cdecl)]
    private static extern int _CryptoHash(byte[] buffer, byte[] message, long length);

    [DllImport("libsodium", EntryPoint = "crypto_hash_sha512", CallingConvention = CallingConvention.Cdecl)]
    private static extern int _SHA512(byte[] buffer, byte[] message, long length);

    [DllImport("libsodium", EntryPoint = "crypto_hash_sha256", CallingConvention = CallingConvention.Cdecl)]
