...
namespace Sodium
{
    ...
    public static class SecretAeadChaCha20Poly1305
    {
        ...
        public static byte[] Encrypt(byte[] message, byte[] nonce, byte[] key, byte[] additionalData = null)
        {
            ...
            var ret = crypto_aead_chacha20poly1305_encrypt(cipher, ref cipherLength, message, (ulong)message.Length, additionalData, (ulong)additionalData.Length, null,
              nonce, key);
        ...
        public static byte[] Decrypt(byte[] cipher, byte[] nonce, byte[] key, byte[] additionalData = null)
        {
            ...
            var ret = crypto_aead_chacha20poly1305_decrypt(message, ref messageLength, null, cipher, (ulong)cipher.Length,
