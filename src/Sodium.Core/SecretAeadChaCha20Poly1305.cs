...
namespace Sodium
{
    ...
    public static class SecretAeadChaCha20Poly1305
    {
        ...
        public static byte[] Encrypt(byte[] message, byte[] nonce, byte[] key, byte[] additionalData = null)
        {
            ...
            var ret = SodiumLibrary.crypto_aead_chacha20poly1305_encrypt(bin, out var cipherLength, message, message.Length, additionalData, additionalData.Length, null, nonce, key);
        ...
        public static byte[] Decrypt(byte[] cipher, byte[] nonce, byte[] key, byte[] additionalData = null)
        {
            ...
            var ret = SodiumLibrary.crypto_aead_chacha20poly1305_decrypt(bin, out var messageLength, null, cipher, cipher.Length, additionalData, additionalData.Length, nonce, key);
